...
class PaymentMethod(ABC):
    ...
    @abstractmethod
    async def generatePayment(self, payment: PaymentType)->PaymentTypeResponse:
        pass
    
class CardMethod(PaymentMethod):

    async def tokenizer(self, card: WompiTokenizerType)->str:
        url = f"{self._link}tokens/cards"
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                resp = await client.post(url=url, json=card.__dict__, headers={
    ...
    async def generatePayment(self, payment: PaymentType)->PaymentTypeResponse:
        ...
        pm:Dict[str, Any] = payload.get("payment_method") or {}
        pm["token"] = token
        pm.pop("phone_number", None)
